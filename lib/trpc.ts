...
const hasAnyRole = (...required: string[]) =>
    t.middleware(({ ctx, next }) => {
        ...
        const needed = new Set(required.flatMap((r) => (r === "doctor" ? ["doctor", "admin"] : [r])));
...
export const adminProcedure = t.procedure.use(isAuthed).use(hasAnyRole("admin"));
export const doctorProcedure = t.procedure.use(isAuthed).use(hasAnyRole("doctor"));
